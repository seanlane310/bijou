...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future fetchUserInfo() async 
  {
    ...
    {
      print("GET HERE?");
      var ref = FirebaseDatabase.instance.reference().child('Customers/$user_ID/Boards/');
                            ...
                            return PopupMenuButton(
                                ...
                                onSelected: (int index) async {
                                    if (index != boards.length-1)
                                    {
                                        ...
                                        FirebaseDatabase.instance.reference().child('Customers/$user_ID/Boards/$board_chosen/$type').update({
                                          '$pin':'true'
                                        });
                                    }
                                    else
                                                      ...
                                                      child: RaisedButton(
                                                        ...
                                                        onPressed: () {
                                                          ...
                                                          else
                                                          {
                                                            ...
                                                            FirebaseDatabase.instance.reference().child('Customers/$user_ID/Boards/$textValue/$type').set({
                                                              '$pin':'true'
                                                            });
                                                            FirebaseDatabase.instance.reference().child('Customers/$user_ID/Boards/$textValue/$deftype').set({
                                                              'default':'true'
                                                            });
                                                            textValue = "";
                                                            Navigator.of(context).pop();
  ...
  Future initially() async 
  {
    return this._memoizer.runOnce(() async 
    {
      if (await FirebaseAuth.instance.currentUser != null) {
        ...
        var refer = FirebaseDatabase.instance.reference().child('Customers/$userID/Following/');
        ...
        print("1.2.RUNNING?!");
        var ref = await FirebaseDatabase.instance.reference().child('Customers/$userID/Searches/');
        ref.once().then((DataSnapshot snapshot) async {
      ...
      await FirebaseFirestore.instance.collection("Products").get().then((querySnapshot) => 
          ...
          await FirebaseFirestore.instance.collection("Businesses").get().then((querySnapshot) => 
          {
              print("4.RUNNING?!"),
              for (int i = 0; i < querySnapshot.docs.length; i++) {
                  //tempSearchStore.add(querySnapshot.docs.elementAt(i).data()),
  ...
  Future getPageInfo() async 
  {
    ...
    //{
      ...
      {
        discoverStore = [];
        var ref = await FirebaseDatabase.instance.reference().child('Customers/$user_ID/Searches/');
        ref.once().then((DataSnapshot snapshot) async {
      ...
      else if (_selections[1] && loGGED_IN)
      {
        ...
        var ref = FirebaseDatabase.instance.reference().child('Customers/$user_ID/Following/');
        await ref.once().then((DataSnapshot snapshot) async {
          ...
          await reff.once().then((DataSnapshot snapshot) async {
            ...
            {
                await FirebaseFirestore.instance.collection("Users").doc(all[ind]).get().then((querySnapshot) => 
                {
          ...
          if (fol_len > 1)
          {
            for (int i =0; i<fol_len; i++)
            {
              ...
              var ref1 = FirebaseDatabase.instance.reference().child('Customers/$currfol/Boards/');
              await ref1.once().then((DataSnapshot snapshot) async {
                ...
                for (int j =0; j<num_boards; j++)
                {
                  ...
                  var ref2 = FirebaseDatabase.instance.reference().child('Customers/$currfol/Boards/$currBoard/Product');
                  ...
                  var ref3 = FirebaseDatabase.instance.reference().child('Customers/$currfol/Boards/$currBoard/Business');
...
Widget _followingBar()
{
  ...
  //{
    ...
    Column(
      children: [
    Text('Follow Suggestions:', textAlign: TextAlign.left, style: TextStyle(decorationStyle: TextDecorationStyle.double,fontWeight: FontWeight.w300, fontSize: 20)),
    Container(
      ...
      itemBuilder: (context, int i) {
            ...
            return 
                ...
                child: new Row(
                  children: <Widget>[
                    new OutlinedButton(
                      ...
                      new ClipRRect(
                        ...
                        child: new Image(image:AssetImage('assets/user.png'),
                            height: 40.0,
                            width: 30.0,
                            //height: 315,
                            //width: 3150,
                  ...
                  new Text(allUsersName[i]['Name'], softWrap: true,style: TextStyle(fontWeight: FontWeight.w500, fontSize: 22))
                ])
              );
            })
          //.toList())
 ...
 //}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      (fol_len!=0 && _selections[1]) ? Container(height: 150, child: _followingBar()): Container(width: 0.0, height: 0.0),
      
      (_selections[1] && loGGED_IN && fol_len ==1) ? Card(
                                  clipBehavior: Clip.antiAlias,
                                  child: Text('Add Followers To See Their Pins Displayed Here', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 20))
                                  ) : (_selections[1] && !loGGED_IN) ? Card(
      clipBehavior: Clip.antiAlias,
      child: Text('Make An Account To Use This Page', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w500, fontSize: 20))
    ):
    FutureBuilder(
      future: initially(),
      builder: (ctx, snapshot) {
